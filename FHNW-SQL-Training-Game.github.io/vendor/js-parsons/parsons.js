...
(function ($, _) {
  ...
  VariableCheckGrader.prototype.grade = function (studentcode) {
    ...
    $.each(parson.options.vartests, function (index, testdata) {
      ...
      var testcaseFeedback = "",
        ...
        actual_value;
      if ("_error" in res) {
        testcaseFeedback += parson.translations.unittest_error(
          that.stripLinenumberIfNeeded(res._error, testdata.initcode, student_code)
        );
        success = false;
        ...
        for (var j = 0; j < variables.length; j++) {
          ...
          if (variable === "__output") {
            ...
            testcaseFeedback += parson.translations.unittest_output_assertion(expected_value, actual_value) + "</div>";
          } else {
            ...
            testcaseFeedback += "<div class='" + (variableSuccess ? "pass" : "fail") + "'>";
            testcaseFeedback +=
              parson.translations.variabletest_assertion(variable, expected_value, actual_value) + "</div>";
          }
  ...
  UnitTestGrader.prototype.grade = function (studentcode) {
    ...
    for (var i = 0, l = result.length; i < l; i++) {
      ...
      if (res.status === "error") {
        // errors in execution
        feedbackHtml += parson.translations.unittest_error(
          this.stripLinenumberIfNeeded(res._error, parson.options.unittest_code_prepend, studentCode)
        );
        success = false;
      } else {
  ...
  langBlocks.pseudo = {
    open: {
      "^s*IF.*THENs*$": "IF",
      "^s*ELSEs*$": "IF", // IF
      "^s*WHILE.*DOs*$": "WHILE", // WHILE
      "^s*REPEAT.*TIMESs*$": "REPEAT..TIMES",
      "^s*REPEATs*$": "REPEAT", // REPEAT ... UNTIL
      "^s*FOR.*DOs*$": "FOR",
      "^s*FOR.*TO.*s*$": "FOR",
      "^s*MODULE.*\\)s*$": "MODULE",
    ...
    if (open && close) {
      ...
      for (i = 0; i < student_code.length; i++) {
        ...
        for (var blockClose in close) {
          if (new RegExp(blockClose).test(line)) {
            ...
            if (!topBlock) {
              ...
              blockErrors.push(
                this.parson.translations.block_close_mismatch(i + 1, close[blockClose], topBlock.line, topBlock.name)
              );
              $("#" + item.id).addClass("incorrectPosition");
            } else if (student_code[i].indent !== topBlock.indent) {
              // incorrect indent
              blockErrors.push(this.parson.translations.no_matching(i + 1));
              $("#" + item.id).addClass("incorrectIndent");
            }
        ...
        if (!isClose && !isOpen && blocks.length > 0) {
          ...
          if ((prevIndent && student_code[i].indent !== prevIndent) || student_code[i].indent <= minIndent) {
            blockErrors.push(this.parson.translations.no_matching(i + 1));
            $("#" + item.id).addClass("incorrectIndent");
          }
  ...
  LineBasedGrader.prototype.grade = function (elementId) {
    ...
    for (i = 0; i < student_code.length; i++) {
      studentCodeLineObjects.push($.extend(true, {}, parson.getLineById(student_code[i].id)));
    }
    ...
    $.each(studentCodeLineObjects, function (index, lineObject) {
      ...
      for (
        var i =
          typeof lastFoundCodeIndex[lineObject.code] !== "undefined" ? lastFoundCodeIndex[lineObject.code] + 1 : 0;
    ...
    if (errors.length === 0) {
      for (i = 0; i < lines_to_check; i++) {
        ...
        if (code_line.indent !== model_line.indent && (!parson.options.first_error_only || errors.length === 0)) {
        ...
        if (code_line.code == model_line.code && code_line.indent == model_line.indent && errors.length === 0) {
  ...
  ParsonsCodeline.prototype._addToggles = function () {
    ...
    if (toggles) {
      ...
      for (var i = 0; i < toggles.length; i++) {
        ...
        html = html.replace(
          toggles[i],
          "<span class='jsparson-toggle' data-jsp-options='" + JSON.stringify(opts).replace("<", "&lt;") + "'></span>"
        );
      }
      this.elem().html(html);
      this.elem()
  ...
  ParsonsWidget.prototype.whatWeDidPreviously = function () {
    ...
    var visits =
      _.filter(this.state_path, function (state) {
        return state == hash;
      }).length - 1;
    var i,
      stepsToLast = 0,
  ...
  ParsonsWidget.prototype.shuffleLines = function () {
    var permutation = (this.options.permutation ? this.options.permutation : this.getRandomPermutation)(
  ...
  ParsonsWidget.prototype.createHTMLFromLists = function (solutionIDs, trashIDs) {
    ...
    if (this.options.trashId) {
      html =
        (this.options.trash_label ? "<p>" + this.options.trash_label + "</p>" : "") +
        this.codeLinesToHTML(trashIDs, this.options.trashId);
      $("#" + this.options.trashId).html(html);
      html =
        (this.options.solution_label ? "<p>" + this.options.solution_label + "</p>" : "") +
