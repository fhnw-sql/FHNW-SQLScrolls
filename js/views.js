...
// Functionality related to the Scrolls - Map switch button in the endgame.
const TaskViewSwap = {
...
// Functionality related to the Books button
...
// Functionality related to the Profile button
const ProfileButton = {
    async show() {
...
// Functionality related to the Leaderboard button
const LeaderboardButton = {
    async show() {
        await showElementImmediately("leaderboard-menu");
    },
    async hide() {
        await hideElementImmediately("leaderboard-menu");
    },
};
...
// Functionality related to the relative leaderboard
const RelativeLeaderboard = {
    async update(globalLeaderboard = true) {
        const allUsers = await API.getUsers(globalLeaderboard);
        let aboveUser, belowUser, currentUser, shortenedUserName;
        const elements = [];
        const userRank = await API.getRank(globalLeaderboard); // Get rank of currently logged-in user
        const userIndex = userRank - 1; // To get the current user's index we deduct 1 from their rank (see getRank()).

        // Check if user is not first place; meaning user index is greater than 0 (first place)
        if (userIndex > 0) {
            aboveUser = allUsers[userIndex - 1];
            shortenedUserName = this.shortenUserName(aboveUser.displayName);
            elements.push({
                element: this.getElement("sorcerer-above"),
        ...
        // No checks about ranks needed for current user
        currentUser = allUsers[userIndex];
        shortenedUserName = this.shortenUserName(currentUser.displayName);
        elements.push({
            element: this.getElement("sorcerer-current"),
            name: shortenedUserName,
            label: "sorcerer-current"
        });

        // Check if user is not last place; meaning user index is greater or equal than the last place minus one.
        if (userIndex < allUsers.length - 1) {
            belowUser = allUsers[userIndex + 1];
            shortenedUserName = this.shortenUserName(belowUser.displayName);
            elements.push({
                element: this.getElement("sorcerer-below"),
                name: shortenedUserName,
                label: "sorcerer-below"
            });
        } else if (allUsers.length === 1) {
            this.getElement("sorcerer-below").innerHTML = i18n.get("relative-empty");
        } else {
            this.getElement("sorcerer-below").innerHTML = i18n.get("relative-motivation");
        }

        // Update elements in DOM
        elements.forEach(({element, name, label}) => {
            if (element) {
                element.innerHTML = `<i aria-label="${i18n.get(label)}"></i> ${name}`;
            }
        });
        const info = this.getElement("rank-info");
        if (info) {
            // Calculates the remaining stars required for a rank-up. If first place (no above user), 0 stars required.
            const stars = aboveUser ? aboveUser.stars - currentUser.stars + 1 : 0;
            this.getElement("rank-info").innerHTML = i18n.getWith("rank-info", [stars]);
        }
        await this.show();
    },
    async show() {
    ...
    async hide() {
        await hideElementImmediately("relative-leaderboard");
    },
    shortenUserName(username) {
        let cleanName = username.includes('@') ? username.split('@')[0] : username;
        return cleanName.length > 5 ? cleanName.slice(0, 15) + '...' : cleanName;
    },
    getElement(element) {
...
class InventoryView extends View {
    ...
    async open() {
        ...
        await API.updateTimeLastActive();
        await RelativeLeaderboard.update(await LeaderboardView.getGlobalLeaderboard());
...
class ProfileView extends View {
    ...
    displayToast(message, type = "success", duration = 3000) {
        const container = document.getElementById("toast-container");

        // Ensure the container is visible
        container.classList.remove("hidden");

        // Check if a toast already exists, and clear it
        let existingToast = container.querySelector(".toast");
        if (existingToast) {
            container.removeChild(existingToast); // Remove the existing toast
        }
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                if (container.contains(toast)) {
                    ...
                    // Hide the container if no toasts remain
    ...
    async open() {
        ...
        document.getElementById("reg-date").innerHTML = i18n.getWith("reg-date", [await this.getRegDate()]);
        document.getElementById("reg-class").innerHTML = await this.renderDisplayClassKey();
        document.getElementById("user-class-input").setAttribute("placeholder", i18n.get("form-control-placeholder"));
        document.getElementById("submit-btn").setAttribute("title", i18n.get("submit-btn"));
        ...
        document.getElementById("enter-class").innerHTML = i18n.get("enter-class");
    ...
    clearToast() {
        ...
        if (toastContainer) {
            toastContainer.innerHTML = ""; // Clear all content
            toastContainer.classList.add("hidden"); // Hide the container
        }
    }
    ...
    async renderDisplayClassKey() {
        return document.getElementById("reg-class").innerHTML = i18n.getWith("reg-class", [await API.getClassKey()]);
    }
    ...
    async getRegDate() {
        ...
        if (data.registrationDate) {
            return new Date(data.registrationDate).toLocaleDateString();  // Create new date object
        } else {
...
class LeaderboardView extends View {
    static globalLeaderboard = true;

    constructor() {
        super("display-leaderboard-modal"); // Extend behavior from super class (View)
        this.autoRefreshInterval = null; // Variable to hold interval ID. Needed to stop a specific interval.
    }

    async open() {
        document.getElementById("your-score").innerHTML = i18n.getWith("your-score", [taskGroups.getCompletedTaskCount()]);
        document.getElementById("leaderboard-table").innerHTML = await this.renderLeaderboardTable(await LeaderboardView.getGlobalLeaderboard());
        document.getElementById("anonymize-btn").innerHTML = i18n.get("anonymize-btn");
        await RelativeLeaderboard.update(await LeaderboardView.getGlobalLeaderboard());
        // replacement value for the hovering the tooltip
        document.getElementById("refresh-leaderboard-btn").setAttribute("title", i18n.get("refresh-leaderboard-hint"));
        document.getElementById("switch-leaderboard-btn").setAttribute("title", i18n.get("switch-leaderboard-hint"));
        document.getElementById("anonymize-btn").setAttribute("title", i18n.get("anonymize-hint"));
        // needed if statement to properly display the leaderboard-title for edge cases when the user switches the classKey
        if (await LeaderboardView.getGlobalLeaderboard()) {
            document.getElementById("leaderboard-title").innerHTML = i18n.get("global-leaderboard");
        } else {
            document.getElementById("leaderboard-title").innerHTML = i18n.getWith("class-leaderboard", [await API.getClassKey()]);
        }

        // Function to refresh leaderboard
        this.refreshLeaderboard = async () => {
            document.getElementById("leaderboard-table").innerHTML = await this.renderLeaderboardTable(await LeaderboardView.getGlobalLeaderboard());
            await RelativeLeaderboard.update(await LeaderboardView.getGlobalLeaderboard());
            console.log("Leaderboard refreshed.");
        };

        // Function to switch leaderboard
        this.switchLeaderboard = async () => {
            await this.setGlobalLeaderboard(!await LeaderboardView.getGlobalLeaderboard()); // Flips the current boolean value
            ...
            document.getElementById("leaderboard-table").innerHTML = await this.renderLeaderboardTable(await LeaderboardView.getGlobalLeaderboard());
            await RelativeLeaderboard.update(await LeaderboardView.getGlobalLeaderboard());
            console.log("Leaderboard switched.");
        };

        // Function to switch display name
        this.anonymize = async () => {
            await API.updatePrivacy();
            await this.refreshLeaderboard();
        };

        // Automatic refresh every 60 seconds (60000 milliseconds). setInterval assigns an ID to autoRefreshInterval.
        this.autoRefreshInterval = setInterval(async () => {
            await this.refreshLeaderboard();
            console.log("Leaderboard automatically refreshed.");
    ...
    async close() {
        $("#" + this.id).modal("hide");
        clearInterval(this.autoRefreshInterval);  // Clear interval to prevent it from running in the background
        this.autoRefreshInterval = null;  // Reset interval ID to "null"
    }

    async renderLeaderboardTable(globalLeaderboard = true) {
        const currentUserDisplayName = await API.getNameOrAlias();
        document.getElementById("your-rank").innerHTML = i18n.getWith("your-rank",
            [await API.getRank(globalLeaderboard), await API.getRank(globalLeaderboard, true)]);
        document.getElementById("aliasNameText").innerHTML = i18n.getWith("aliasNameText",
            [currentUserDisplayName]);
        document.getElementById("top-three-scorers").innerHTML = i18n.getWith("top-three-scorers",
            [await API.getTopThree(globalLeaderboard)]);

        // Retrieve the list of usernames
        const users = await API.getUsers(globalLeaderboard);
        ...
        <div class="scrollable-table-container">
          <table class="leaderboard-table">
            <thead>
              <tr>
                <th class="text-center">${i18n.get("leaderboard-table-header-1")}</th>
                <th class="text-center">${i18n.get("leaderboard-table-header-2")}</th>
                <th class="text-left">${i18n.get("leaderboard-table-header-3")}</th>
                <th class="text-center">${(i18n.get("leaderboard-table-header-4"))}</th>
              </tr>
            </thead>
          </table>
          <!-- Scrollable tbody -->
        ...
        // Medal emojis for the top 3 ranks
        const medals = ["🥇", "🥈", "🥉"];

        users.forEach((displayName, index) => {
            const currentUser = users[index];
            const isCurrentUser = currentUser.displayName === currentUserDisplayName;

            // Determine the medal or leave the column empty for non-top-3 rows
            const medal = index < 3 ? medals[index] : "";

            // Add table content, with a special class if it's the current user
            table += `
            <tr class="${isCurrentUser ? 'highlight-user' : ''}">
              <td class="text-center">${index + 1}</td>
              <td class="text-center">${medal}</td>
              <td>${currentUser.displayName}</td>
              <td class="text-center">${currentUser.stars}</td>
            </tr>
            `;
        });
    ...
    static async getGlobalLeaderboard() {
        return LeaderboardView.globalLeaderboard;
    }

    async setGlobalLeaderboard(value) {
        LeaderboardView.globalLeaderboard = value;
    }
}
...
class LoadingView extends View {
    ...
    async open() {
        ...
        // Update stars upon finishing loading
        const completedTasks = taskGroups.getCompletedTaskCount();
        await API.updateStars(completedTasks);
    }
